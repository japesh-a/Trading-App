...
const port=Number(process.env.WICKLUME_PORT)||5173;
...
http.createServer(async(req,res)=>{
 ...
 const file=({'/':'index.html','/app.js':'app.js','/lesson-visuals.js':'lesson-visuals.js','/style.css':'style.css'})[url.pathname];if(!file){res.writeHead(404);return res.end('Not found');}try{const content=readFileSync(path.join(root,'public',file));res.writeHead(200,{'Content-Type':file.endsWith('.css')?'text/css':file.endsWith('.js')?'text/javascript':'text/html','Cache-Control':'no-store'});res.end(content)}catch{if(!res.headersSent){res.writeHead(500);res.end('Unable to load page')}}
}).listen(port,'127.0.0.1',()=>console.log(`Wicklume live preview: http://localhost:${port}`));
